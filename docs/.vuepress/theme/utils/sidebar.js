...
const resolveSidebarItem = (sidebarConfigItem, pages, base, prefix = '') => {
  ...
  if (children.length === 0 && sidebarConfigItem.path)
    // cover title
    return Object.assign(
      resolvePageforSidebar(pages, resolve(prefix, sidebarConfigItem.path, base)),
      { title: sidebarConfigItem.title }
    )
  //  resolve children recursively then return
  return Object.assign(Object.assign({}, sidebarConfigItem), {
    type: 'group',
    path: sidebarConfigItem.path ? resolve(prefix, sidebarConfigItem.path, base) : '',
    children: children.map((child) =>
      resolveSidebarItem(child, pages, base, `${prefix}${sidebarConfigItem.prefix || ''}`)
    ),
...
export const getSidebarItems = (page, site, localePath) => {
  ...
  return result ? result.config.map((item) => resolveSidebarItem(item, pages, result.base)) : []
}
